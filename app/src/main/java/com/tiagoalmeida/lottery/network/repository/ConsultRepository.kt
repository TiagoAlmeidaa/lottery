...
class ConsultRepository(
    ...
    suspend fun consultAll(): List<LotteryResult> {
        ...
        executeParse(dataSource.consultContest(LotteryType.MEGASENA)) { response -> results.add(response) }
        executeParse(dataSource.consultContest(LotteryType.LOTOFACIL)) { response -> results.add(response) }
        executeParse(dataSource.consultContest(LotteryType.LOTOMANIA)) { response -> results.add(response) }
        executeParse(dataSource.consultContest(LotteryType.QUINA)) { response -> results.add(response) }
    ...
    suspend fun consultContest(type: LotteryType, contestNumber: Int): LotteryResult? {
        ...
        executeParse(dataSource.consultContestByNumber(type, contestNumber)) { response -> contest = response }

...
fun buildRemoveGameDialog(context: Context, parent: ViewGroup?, onClick: () -> Unit): AlertDialog {
    ...
    with(view) {
        findViewById<CustomButtonCardView>(R.id.button_remove)?.let { button ->
        ...
        findViewById<CustomButtonCardView>(R.id.button_cancel)?.let { button ->
            button.setOnClickListener {
                dialog.dismiss()
            }
        }
...
fun buildFilterGameDialog(context: Context, parent: ViewGroup?, onClick: (String) -> Unit): AlertDialog {
    val inflater = LayoutInflater.from(context)
    ...
    with(view) {
        ...
        findViewById<CustomButtonCardView>(R.id.button_compare)?.let { button ->
            button.setOnClickListener {
